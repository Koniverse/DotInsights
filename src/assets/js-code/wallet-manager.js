(
	function( $ ) {
		...
		async function switchingAccount( $thisButton ) {
			const accountList = await walletUtils.getAccounts();

			// Get account list.
			var newAccount = $thisButton.data( 'address' );

			for ( var i = 0; i < accountList.length; i ++ ) {
				if ( newAccount === accountList[ i ].address ) {
					walletUtils.enableAccount( newAccount );

					$thisButton.siblings().removeClass( 'selected-account' );
					$thisButton.addClass( 'selected-account' );
		...
		async function votingProject( $thisButton ) {
			var projectID = $thisButton.data( 'project-id' );
